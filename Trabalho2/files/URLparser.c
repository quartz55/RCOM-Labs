...
const char* regExp = "ftp://([([A-Za-z0-9])*:([A-Za-z0-9])*@])*([A-Za-z0-9.~-])+/([[A-Za-z0-9/~._-])+";;

const char* regExpUserAnonymous = "ftp://([A-Za-z0-9.~-])+/([[A-Za-z0-9/~._-])+";
...
int validateURL(const char *URL,int *userMode,char *currentRegex,size_t nmatch,regmatch_t *pmatch){
	if (URL[6] == '[') {
		*userMode = 1;
