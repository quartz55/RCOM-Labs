...
int ftpConnect(ftp* ftp, const char* ip, int port) {
	...
	if (ftpRead(ftp, ftpMessage, sizeof(ftpMessage))) {
...
int ftpLoginHost(ftp* ftp, const char* user, const char* password) {
	char sd[1024];
...
int ftpDownloadFile(ftp* ftp, const char* filename) {
	FILE* file;
	int bytes;
